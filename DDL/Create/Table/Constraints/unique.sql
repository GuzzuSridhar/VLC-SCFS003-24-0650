...
CREATE TABLE IF NOT EXISTS test_unique_column (
    ...
    -- custom name to the constraint
        CREATE TABLE IF NOT EXISTS test_custom_constraint_name (
    employee_id INT,
    first_name VARCHAR(255),
    last_name VARCHAR(255),
    unique key my_Constraint (first_name,last_name) -- table level composite constraint with a custom name
    );